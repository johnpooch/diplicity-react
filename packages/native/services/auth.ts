...
class AuthService implements IAuthService {
  constructor() {
    ...
    if (!googleClientId) {
      throw new Error(
        'Google Client ID not found in environment variables. Please set GOOGLE_OAUTH_CLIENT_ID in your .env file.'
      );
    }
  ...
  public async getServerAuthCode() {
    try {
      ...
      if (error instanceof Error) {
        telemetry.logError(`Error thrown while signing in: ${error.message}`);
      } else {
