...
**Calls to action are a field, never prose.** The body states the event; the tap target is `get_link()`. An inline "Respond to it now" duplicates a control the player already has, and reads as nagging in a tray.
