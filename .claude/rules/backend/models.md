...
**Keep per-row correlated annotations off a paginated list queryset.** The base queryset is what filtering, ordering and the pagination `COUNT` run over, so a correlated subquery on it (per-user unread counts, for example) runs in the `COUNT` too, and a grouped ordering can run it once per joined row. Compute the value in one set-based query over the page's ids once the page is known, and attach it to every instance, zero included, so the serializer never falls back to a per-row query. `GameManager.hydrate_total_unread_counts()` is the pattern, called from `GameListView.paginate_queryset()` beside the phase hydration. Hydration fills only instances that do not already carry the value, so a filter that guarantees the answer annotates it on the queryset and the query is skipped: `filter_can_join()` annotates zero with `with_zero_unread_counts()`. Do not re-parse filter params in the view to decide.
