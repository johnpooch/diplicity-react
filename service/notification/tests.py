...
from email_service.utils import NO_EMAIL_ADDRESS_ERROR
...
def _email_delivery(user, status=NotificationDelivery.Status.PENDING):
    notification = Notification.objects.create(recipient=user, event_type="game_start")
    return NotificationDelivery.objects.create(
        notification=notification,
...
class TestNotificationDeliver:
    ...
    @pytest.mark.django_db
    def test_undispatched_email_recipient_is_marked_failed_with_the_reason(
        self, user_factory, mock_send_notification_to_users
    ):
        user = user_factory()
        delivery = _email_delivery(user)

        with patch(
            "email_service.utils.send_email_to_users",
            side_effect=lambda user_ids, **kwargs: {user_id: NO_EMAIL_ADDRESS_ERROR for user_id in user_ids},
        ):
            deliver(delivery_ids=[delivery.id])

        delivery.refresh_from_db()
        assert delivery.status == NotificationDelivery.Status.FAILED
        assert delivery.error == NO_EMAIL_ADDRESS_ERROR

    @pytest.mark.django_db
    def test_email_and_push_recipients_are_recorded_independently(
        self, user_factory, mock_send_notification_to_users
    ):
        user = user_factory()
        push = _push_delivery(user)
        email = _email_delivery(user)

        with patch(
            "email_service.utils.send_email_to_users",
            side_effect=lambda user_ids, **kwargs: {user_id: NO_EMAIL_ADDRESS_ERROR for user_id in user_ids},
        ):
            deliver(delivery_ids=[push.id, email.id])

        push.refresh_from_db()
        email.refresh_from_db()
        assert push.status == NotificationDelivery.Status.SENT
        assert email.status == NotificationDelivery.Status.FAILED
