...
def muster_game_factory(db, primary_user, secondary_user, italy_vs_germany_variant):
    def _create(muster_required=True, private=False, fill=True, **kwargs):
...
class TestFillTransition:
    ...
    def test_full_game_without_muster_required_starts(
        ...
        game = muster_game_factory(muster_required=False, private=True)
    ...
    @pytest.mark.django_db
    def test_private_game_with_muster_required_enters_mustering(
        self, muster_game_factory, in_memory_procrastinate
    ):
        game = muster_game_factory(private=True)
...
class TestArming:
    ...
    def test_reminder_is_not_rearmed_after_it_has_run(
        ...
        game = muster_game_factory(
            movement_phase_duration=MovementPhaseDuration.ONE_HOUR
        )
        Game.objects.filter(pk=game.pk).update(
            muster_deadline=timezone.now() + timedelta(minutes=5)
        )
        game.refresh_from_db()

        reminder_id_before = game.muster_reminder_job_id
        game.save()

        game.refresh_from_db()
        assert game.muster_reminder_job_id is None
        if reminder_id_before is not None:
            assert _job(reminder_id_before).status == MusterJob.CANCELLED
...
class TestStatusAudit:
    ...
    def test_game_master_can_delete_a_mustering_game(
        self, db, primary_user, secondary_user, tertiary_user, italy_vs_germany_variant,
        in_memory_procrastinate, authenticated_client,
    ):
        game = Game.objects.create_from_template(
            italy_vs_germany_variant,
            name="GM Muster Game",
    ...
    def test_account_deletion_returns_mustering_games_to_pending(
        self, db, primary_user, user_factory, authenticated_client_factory,
        italy_vs_germany_variant, in_memory_procrastinate,
    ):
...
class TestGameCreateMusterRequired:
    ...
    @pytest.mark.django_db
    def test_public_games_coerce_muster_required_on(
        self, authenticated_client, italy_vs_germany_variant, in_memory_procrastinate
    ):
        response = authenticated_client.post(
            reverse("game-create"),
            self._payload(italy_vs_germany_variant.id, muster_required=False),
            format="json",
        )
